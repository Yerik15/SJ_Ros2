/***
 * @Author: _yerik
 * @Date: 2025-07-21 11:08:36
 * @LastEditTime: 2025-07-21 13:35:52
 * @LastEditors: _yerik
 * @Description:
 * @FilePath: /Simple_Joint/SJ_ws/SJ_Ros2/SJ_hw/src/damiao.cpp
 * @Code. Run. No errors.
 */
...
Motor_Control::Motor_Control(std::string serial_port,int baud_rate,
  ...
  for (auto it = data_ptr_->begin(); it != data_ptr_->end();
       ++it) { // 遍历该port下的所有电机
    Motor *motor =
        new Motor(it->second.motorType, it->second.can_id,
                  it->second.mst_id); // 假设Motor的构造函数不需要参数
    addMotor(motor);
  }
  try {
...
void Motor_Control::write() {
  for (const auto &m : *data_ptr_) {
    ...
    /* std::cerr<<"write"<<std::endl;
     ...
     std::cerr<<"pos: "<<m.second.cmd_pos<<" vel: "<<m.second.cmd_vel<<" effort:
     "<<m.second.cmd_effort<<std::endl; std::cerr<<"kp: "<<m.second.kp<<" kd:
     "<<m.second.kd<<std::endl; std::cerr<<"finishe write"<<std::endl;*/
    control_mit(*it, m.second.kp, m.second.kd, m.second.cmd_pos,
                m.second.cmd_vel,
                m.second.cmd_effort); // control_mit(Motor &DM_Motor, float kp,
                                      // float kd, float q, float dq, float tau)
  }
}
...
void Motor_Control::get_motor_data_thread() {
  while (!stop_thread_) {
          ...
          receive_data.frameEnd == 0x55) // receive success
      {
        ...
        float receive_tau =
            uint_to_float(tau_uint, -limit_param_receive.TAU_MAX,
...
bool Motor_Control::ReadData(CAN_Receive_Frame& frame, const size_t timeout_ms){
    ...
    try {
        ...
        if (rx_buffer.size() == bytes_to_read) {
            memcpy(&frame, rx_buffer.data(), bytes_to_read);
            return true; 
        } else {
          //字节数不符
            std::cerr << "Incomplete frame received. Expected " << bytes_to_read
                      << ", got " << rx_buffer.size() << std::endl;
            return false;
        }
        ...
        // 通用捕获
        std::cerr << "An exception occurred during serial read: " << e.what() << std::endl;
        return false;
    }
  }
